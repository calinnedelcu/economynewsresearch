...
def table_A1_cluster_gap_robustness():
    ...
    body.append(r"\begin{table}[!htbp]")
    body.append(r"\centering")
    body.append(r"\caption{Descriptive cluster-gap-threshold robustness for the H1 magnitude ratio.}")
    ...
    body.append(r"\begin{tablenotes}")
    body.append(r"\footnotesize")
    body.append(r"\item \textit{Notes.} Descriptive H1 event/baseline magnitude ratio")
    body.append(r"at four cluster-gap thresholds. The 15-minute column corresponds to")
    body.append(r"the specification used in the main body. Tighter thresholds")
    body.append(r"(5--10 min) retain more independent clusters and yield slightly")
    body.append(r"higher ratios; a wider threshold (30 min) compresses the sample but")
    body.append(r"the central finding survives in every cell. The baseline pool is held")
    body.append(r"fixed across gap choices, so the table is interpreted as descriptive")
    body.append(r"robustness rather than a full re-estimation of all H1 test statistics.")
    body.append(r"\end{tablenotes}")
    body.append(r"\end{threeparttable}")
    body.append(r"\end{table}")
