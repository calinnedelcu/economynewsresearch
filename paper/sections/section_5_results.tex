...
The central magnitude result (Section~\ref{ssec:results-volatility})
...
after FDR adjustment, and economically modest. The
...
shortest EUR/USD window (1 minute). All ten cells reach $q < 10^{-7}$
...
NDX. In economic terms at the $+15$-minute window, the mean
absolute return is $5.3$ pips on EUR/USD (versus $3.1$ pips for the
matched baseline) and $14.4$ basis points on NDX (versus $9.2$
basis points). The marginal effect of being inside an event window
is therefore on the order of $2.2$ pips for EUR/USD and $5.2$ basis
points for NDX over the first quarter hour. See
Table~\ref{tab:h1-main} and Figure~\ref{fig:event-time-profile},
which plots the average absolute log-return as a function of
minutes from the event timestamp; both assets show a sharp lift in
the post-event window that persists for at least four hours, with
the EUR/USD ratio larger at short horizons and the NDX ratio larger
at longer horizons.

\input{tables/table_5_h1_main.tex}
...
the same window. The intra-window range and maximum absolute move
are therefore more informative outcomes than the close-to-close
return for characterising the magnitude response.
...
pattern on a dimensionless scale. On the primary $5/15/60$-minute
windows, the mean $|z|$ on the close-to-close return is in the
range $0.57$--$0.91$ across assets and windows; the mean $|z|$ on
range is in the range $1.04$--$1.53$; and the mean $|z|$ on the
maximum absolute move is in the range $0.92$--$1.31$. The signed
return $z$-scores are not significantly different from zero in
most cells, reinforcing the finding that the news effect is on
magnitude rather than on direction.

\subsection{Direction: modest, but benchmark-validated}
...
small. Hit rates in the 49--54\% range would be insufficient to
overcome typical intraday transaction costs on either asset; a
formal cost-aware evaluation is left to future work.
...
windows, shows that the LLM outperforms the dictionary baseline on
the majority of the primary windows and is competitive with the
neural baseline. Against Loughran-McDonald, the LLM
...
(versus $26.4\%$ for the LLM USD labels and $20.2\%$ for the LLM
NDX labels), substantially reducing its effective directional
sample size and contributing little incremental signal where it
does commit to a direction.
...
\paragraph{External benchmark validation.} The relative comparison
above is suggestive but does not pin down the absolute classifier
quality. Table~\ref{tab:external-benchmark} reports the performance
of all three classifiers on the Financial PhraseBank benchmark
\citep{malo2014good}, the standard external evaluation in finance
sentiment NLP and the same dataset \citet{yang2020finbert} use to
validate FinBERT. On the 4{,}840 sentences of the
\texttt{sentences\_50agree} subset, DeepSeek achieves
$84.6\%$ accuracy, $0.836$ macro-$F_1$, and Cohen's $\kappa = 0.717$
against the gold human consensus, outperforming FinBERT
($79.2\%$ accuracy, $0.751$ macro-$F_1$, $\kappa = 0.600$) by
$5.4$ percentage points on accuracy and $0.12$ on $\kappa$. The
Loughran-McDonald dictionary trails substantially
($57.4\%$ accuracy, $\kappa = 0.218$), which is consistent with
the well-documented difficulty single-word polarity lookups have
on syntactically rich financial text. The DeepSeek macro-$F_1$ of
$0.836$ is in line with the upper end of the range
\citet{yang2020finbert} report for FinBERT-tone on the same
dataset. The classifier outperforms the dictionary and modestly
outperforms the BERT-era neural baseline on an independent,
peer-reviewed benchmark, complementing the relative hit-rate
comparison on our event sample.

\input{tables/table_9_external_benchmark.tex}

\input{tables/table_6_baseline_compare.tex}
...
Section~\ref{ssec:results-volatility} holds broadly: standardised
abnormal $z$-scores on the maximum absolute move are positive in
every category-asset-window cell we tested, and the one-sided
$t$-test remains significant at $q < 0.05$ in all cells. The
directional content is more mixed: central-bank events on the NDX
15-minute window achieve a hit rate of $55.8\%$ with $q = 0.058$,
while the small energy EUR/USD 15-minute cell reaches $66.7\%$
with $q = 0.039$ on only 42 directional clusters. The pattern is
that category-level
...
effect is not statistically significant. NDX shows a weaker
hour-of-day effect under the Kruskal-Wallis test ($q = 0.032$) but
not under the ANOVA specification, and shows no day-of-week effect
that survives FDR correction.
...
($q = 7.5\times10^{-47}$), NDX $|r| = 0.158$ versus baseline $0.092$
($q = 2.2\times10^{-40}$). The pre-event drift is on the same order of
...
adjudicate between these. The implication for the event-study
interpretation is that the standard assumption that the public
publication time coincides with the informational event time is
materially violated in this setting.
...
$+0.148$; the same-sign rate is $43.4\%$ and the opposite-sign rate
is $56.6\%$ across $1{,}176$ clusters ($q < 10^{-5}$ for deviation
from a $50\%$ split). We report both price and target-aligned
conventions to forestall
...
magnitude findings hold on both halves, with maximum-move tests
remaining below $q < 10^{-13}$ on the primary windows and
absolute-return tests also positive but somewhat less extreme in
the post-cutoff half. On the pre-cutoff half, mean $|z|$ for the absolute
...
\input{tables/table_7_multivariate.tex}

\input{tables/table_8_pre_post_cutoff.tex}

\paragraph{C7 (winsorisation at the 1\% tails).}
Winsorising every outcome at the $1\%$ tails produces mean $|z|$
values that are mostly within about $5$--$13\%$ of the raw mean on
the primary windows and $p$-values that remain below $10^{-20}$ in
the central cells. The headline
...
\input{tables/table_11_winsorisation.tex}
