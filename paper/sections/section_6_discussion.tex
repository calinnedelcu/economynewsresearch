...
largely below transaction costs
...
The hit-rate advantage of the LLM-derived sentiment over both
baselines is in the 2--6 percentage point range on the primary
windows (Section~\ref{ssec:results-direction}), which is not a
dramatic improvement on the headline metric. The qualitative
...
headline is elevated above the matched baseline at $q < 10^{-40}$
on both assets, with a magnitude comparable to the post-event drift
on the same window.
...
directional results beyond the present 2--6 pp margin. We do not
