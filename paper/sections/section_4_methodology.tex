...
seven extensions (C1--C7). For reasons of space we report the main
...
A further five tests are reported only in Section~\ref{sec:limitations}
...
(H12) is reported as a null finding.
...
two baselines and evaluate it on an external finance-sentiment
benchmark.
...
\paragraph{Human validation (future work).} Double-coded expert
annotation of the FinancialJuice event sample remains a follow-up
extension rather than part of the present release. Until that work
is completed, the external-benchmark validation described above and
the multi-classifier comparison on our sample serve as the primary
