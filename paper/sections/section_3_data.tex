...
fields per event: a discrete USD sentiment in
\{\texttt{bull}, \texttt{bear}, \texttt{neutral}\}, a discrete NDX
sentiment in the same support, signed directional strengths in
$[-1,+1]$ for each asset, an expected magnitude in
\{\texttt{low}, \texttt{med}, \texttt{high}\}, a surprise level in
\{\texttt{expected}, \texttt{surprise}, \texttt{shock}\}, a
...
prompt iteration do not duplicate API costs.
...
(no weekend trading, exchange holidays). Closed-period gaps are
