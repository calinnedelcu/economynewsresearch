...
\texttt{bull USD, bear NDX} simultaneously. Eight few-shot examples
covering geopolitical, central bank, corporate, commodity-shock,
and neutral or low-relevance cases are included in the chat prompt
to anchor the output format. Calls are cached locally in SQLite,
keyed on the hash of the model name, system prompt, and event
content, so that re-runs during prompt iteration do not duplicate
API costs.\footnote{We set the API sampling temperature to $0.1$.
The released SQLite cache is therefore the fixed replication input
for the reported classifications; fresh API reruns from the
published prompt may vary marginally with model-serving changes.}
...
\input{tables/table_1_sample_summary.tex}
\input{tables/table_2_variable_definitions.tex}
\input{tables/table_3_descriptive_stats.tex}
...
\input{tables/table_4_sentiment_distribution.tex}
